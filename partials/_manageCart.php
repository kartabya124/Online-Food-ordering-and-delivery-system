...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['checkout'])) {
        ...
        $resturantid=$_SESSION['resid'];
        
        $passSql = "SELECT * FROM user WHERE id='$userId'"; 
        $passResult = mysqli_query($conn, $passSql);
        $passRow=mysqli_fetch_assoc($passResult);
        $userName = $passRow['username'];
        if ($password==$passRow['password']){ 
            $sql = "INSERT INTO `orders` (`userId`, `address`, `phoneNo`, `amount`, `paymentMode`, `orderStatus`, `orderDate`,`resturantid`) VALUES ('$userId', '$address', '$phone', '$amount', '0', '0', current_timestamp(),'$resturantid')";
            $result = mysqli_query($conn, $sql);
