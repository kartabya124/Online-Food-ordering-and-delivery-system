				...
				<div class="card">
					<div class="card-body">
						<table class="table table-bordered table-hover mb-0">
							...
							<tbody>
                            <?php
                          	 $sql = "SELECT * FROM `food`WHERE uid='$userId'";
                                ...
                                while($row = mysqli_fetch_assoc($result)){
                                    ...
                                    <tr>
                                            ...
                                            <td>
                                                <img src="../img/<?=$row['image'];?>" alt="image for this item" width="150px" height="150px">
                                            </td>
                                            <td>
...
<?php 
    $pizzasql = "SELECT * FROM `food`";
    $pizzaResult = mysqli_query($conn, $pizzasql);
    while($pizzaRow = mysqli_fetch_assoc($pizzaResult)){
        $foodId = $pizzaRow['foodId'];
        $foodName = $pizzaRow['foodName'];
        $foodPrice = $pizzaRow['foodPrice'];
        $uid= $pizzaRow['uid'];
        $foodDesc = $pizzaRow['foodDesc'];
?>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color: rgb(111 202 203);">
        <h5 class="modal-title" id="updateItem<?php echo $pizzaId; ?>">Item Id: <?php echo $foodId; ?></h5>
      ...
      <div class="modal-body">
		...
		<form action="partials/_menuManage.php" method="post">
			...
			<div class="text-left my-2 row">
				...
				<div class="form-group col-md-6">
					<b><label for="catId">ResturantId</label></b>
                	<input class="form-control" id="catId" name="catId" value="<?php echo $uid; ?>" type="number" min="1" required>
				</div>
            </div>
            <div class="text-left my-2">
